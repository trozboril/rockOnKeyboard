$(document).ready(function() {
   ...
   var sounds = [
   	...
   	for (var i = 0; i < sounds.length; i++) {
   			var idClass;
   			if(count % 2 === 0) {
   				idClass = 'id="'+(sounds[i].num)+'" class="indKey grey"';
   			} else {
   				idClass = 'id="'+(sounds[i].num)+'" class="indKey"';
   			}
   			appendKey(idClass);
   			keyPress(sounds[i].num.toString());
   		count++;
   	}
   	...
   	function appendKey (idClass){
	    ...
	    $(this).keydown(function(event) {
	    	for (var i = 0; i < sounds.length; i++){
		        if(event.keyCode === parseInt(key)) {
		        	if(sounds[i].pressed === false) {
			        	$('#' + key).css("background-color", "lightblue");
			        	if (sounds[i].num.toString() === key){
			        	$('#' + key).html('<audio src="' + sounds[i].sound + '" autoplay></audio>');
			        	sounds[i].pressed = true;
			        	}
		        	}
	    		}
	    	}
	    ...
	    $(this).keyup(function(event) {
	        ...
	        for (var n = 0; n < sounds.length; n++) {
	        	if(sounds[n].num.toString() === key) {
	        		sounds[n].pressed = false;
	        	}
